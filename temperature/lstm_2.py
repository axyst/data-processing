...
series = pd.read_csv('dataA1_0515-19_temperature.csv',
...
train_size = int(len(supervised_values) * 0.75)
test_size = len(supervised_values) - train_size
train, test = supervised_values[0:train_size,
                                :], supervised_values[train_size:len(supervised_values), :]
...
for i in range(len(test_scaled)):
    ...
    print('time=%d, Predicted=%f, Expected=%f' % (i, yhat, expected))

# report performance
rmse = np.sqrt(mean_squared_error(raw_values[train_size:], predictions))
